...
use structs::{CustomSystemAttr, SystemInput};
use syn::{parse::{Parse, Parser}, Expr};

pub mod structs;

pub(crate) fn parse_system(input: &SystemInput) -> syn::Result<Expr> {
    input.attr.require_transforms().cloned()
}

pub(crate) fn macro_impl(attr: TokenStream1, item: TokenStream1) -> syn::Result<TokenStream2> {
    let input = SystemInput::parse_with_attr(CustomSystemAttr::parse.parse(attr)?).parse(item)?;
    let body = &input.body;
